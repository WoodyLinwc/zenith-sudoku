...
function App() {
  ...
  useEffect(() => {
    if (!grid.some(row => row.includes(0)) && mistakes < 3) {
      ...
      const isCorrect = grid.every((row, r) => row.every((val, c) => val === solvedGrid[r][c]));
      if (isCorrect) {
        setIsWon(true);
        setIsTimerActive(false);
      }
    }
  }, [grid, solvedGrid, mistakes]);
  ...
  const handleNumberInput = (num: number) => {
    ...
    // Save history
    setHistory(prev => [...prev.slice(-20), JSON.parse(JSON.stringify(grid))]); 
  ...
  const getCellProps = (row: number, col: number) => {
    ...
    if (selectedCell) {
      ...
      isRelated = (row === sRow || col === sCol || (sBoxRow === cBoxRow && sBoxCol === cBoxCol));
    }

    const isSameValue = selectedCell && grid[selectedCell.row][selectedCell.col] !== 0 && value === grid[selectedCell.row][selectedCell.col];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">Zenith Sudoku</h1>
          </div>
          {isWon && (
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col lg:flex-row gap-8 lg:gap-12 items-start justify-center">
        ...
        {/* Board Section */}
        <div className="w-full max-w-md mx-auto lg:mx-0 shadow-xl rounded-xl overflow-hidden border-4 border-slate-800 bg-slate-800">
          <div className="grid grid-cols-9 gap-[1px] bg-slate-300">
            {grid.map((row, rIndex) => (
              row.map((_, cIndex) => (
                <SudokuCell key={`${rIndex}-${cIndex}`} {...getCellProps(rIndex, cIndex)} />
              ))
            ))}
          </div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto lg:mx-0 lg:w-80 flex-shrink-0">
           ...
           <div className="mt-8 p-4 bg-indigo-50 rounded-xl border border-indigo-100 text-sm text-indigo-800">
              ...
              <p className="opacity-80">Fill the grid so that every row, column, and 3x3 box contains the digits 1 through 9. Good luck!</p>
           </div>
        </div>
